...
async function getProducts(req: NextRequest) {
    try {
        ...
        const [productsWithDetails, total] = await Promise.all([
            Product.aggregate([
                { $match: filter },
                { $sort: sort },
                { $skip: skip },
                ...
                },
                {
                    $addFields: {
                        ...
                        // --- START OF FIX ---
                        // Count ONLY general design files, not order-specific ones.
                        designFilesCount: {
                            $size: {
                                $filter: {
                                    input: '$designFiles',
                                    as: 'file',
                                    cond: { $eq: ['$$file.isForOrder', false] }
                                }
                            }
                        }
                        // --- END OF FIX ---
                ...
                { $project: { designFiles: 0 } } // Don't send the full array of files to the client
            ]),
            Product.countDocuments(filter)
        ]);
        ...
        console.error('Get products error:', error);
        if (error instanceof z.ZodError) {
            return NextResponse.json({ success: false, message: 'Invalid query parameters', errors: error.issues }, { status: 400 });
        }
...
async function createProduct(req: NextRequest, _context: ApiRouteContext, user: SessionUser) {
    try {
        ...
        if (existingProduct) {
            return NextResponse.json({ success: false, message: 'Product with this name or slug already exists' }, { status: 409 });
        }
        ...
        const images = validatedData.images.map((img, index) => ({ ...img, isPrimary: index === 0 }));
        const finalPrice = calculateFinalPrice(validatedData.price, validatedData.discountAmount, validatedData.discountPercentage);
        ...
        return NextResponse.json({ success: true, message: 'Product created successfully', data: sanitizeProductForResponse(product) }, { status: 201 });

    } catch (error) {
        console.error('Create product error:', error);
        if (error instanceof z.ZodError) {
            return NextResponse.json({ success: false, message: 'Invalid input data', errors: error.issues }, { status: 400 });
        }
